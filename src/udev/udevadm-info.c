...
static int print_record(sd_device *device, const char *prefix) {
        ...
        if (sd_device_get_parent_subsystem(device, &str) >= 0)
...
static int record_to_json(sd_device *device, sd_json_variant **ret) {
         ...
         * PARENT_SUBSYSTEM, and DEVICE_ID, as all the other ones have a matching property which will already
         * be included. */
        ...
        if (sd_device_get_parent_subsystem(device, &str) >= 0) {
                r = sd_json_variant_set_field_string(&v, "PARENT_SUBSYSTEM", str);
                if (r < 0)
                        return r;
        }
